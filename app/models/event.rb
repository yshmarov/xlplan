class Event < ApplicationRecord
  ...
  def sms_text
    ...
    time = starts_at.strftime("%A %d/%b/%Y %H:%M").to_s
    location = workplace.location.to_s
    address = workplace.location.address_line.to_s
  ...
  def update_other_prices
    if id?
      if jobs.any?
        ...
        jobs.each { |job| job.update_due_prices }
        members.each { |member| member.update_events_balance }
