...
class MachinesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create machine" do
    assert_difference('Machine.count') do
      post machines_url, params: { machine: { location_id: @machine.location_id, name: @machine.name, status: @machine.status } }
    end
  ...
  test "should update machine" do
    patch machine_url(@machine), params: { machine: { location_id: @machine.location_id, name: @machine.name, status: @machine.status } }
